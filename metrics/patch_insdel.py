...
class PatchInsertionDeletion(Metric):
    ...
    def generate_insdel_images(self, mode: str, mask_mode: str="black"):
        ...
        for i in range(num_insertion):
            ...
            if mask_mode in ["blur", "base"]:
                ...
                if mode == "insertion":
                    mask = np.where(threthold <= self.patch_attention, 1.0, 0.0)
                elif mode == "deletion":
                    mask = np.where(threthold <= self.patch_attention, 0.0, 1.0)
    ...
    def inference(self):
        inputs = torch.Tensor(self.input) # (52, 1, 512, 512:patch=16)
        params = get_parameter_depend_in_data_set(self.dataset)
