...
class model_CNN_3(nn.Module):
    def __init__(self, num_channels: int, num_classes: int=2):
        ...
        downsample = nn.Sequential(
            conv1x1(16, 8 * 4, stride=2),
            nn.BatchNorm2d(8 * 4),
        )
        self.layer1 = Bottleneck(
            16, 8, stride=2, downsample=downsample, norm_layer=nn.BatchNorm2d, size=28
        )
        ...
        self.layer2 = Bottleneck(
            32, 16, 2, downsample=downsample2, norm_layer=nn.BatchNorm2d, size=28
        )
